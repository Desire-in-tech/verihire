...
3. Output-only shapes produced by the rules engine and the mock Midnight
   layer: CriterionResult, MatchResult, ProofResult.

Why the AI-shared models are duplicated here instead of imported from the
ai_service package: the two services are meant to be independently
deployable (that's the point of splitting them), so they don't share a
Python import - they agree on a JSON contract instead, which is exactly
what SCHEMA_CONTRACT.md documents.
"""
...
class DisclosureLevel(str, Enum):
    """
    The three progressive-disclosure stages described in the pitch doc.
    A candidate starts ANONYMOUS and only moves forward when they choose to.
    """
...
class CVSubmission(BaseModel):
    """What a client sends us to register a new candidate."""
...
# ---------------------------------------------------------------------------
# Matching (rules engine output) and mock Midnight proof
# ---------------------------------------------------------------------------
...
class ProofResult(BaseModel):
    """
    Mocked stand-in for a real Midnight ZK proof. Shaped so that swapping
    this out for a real Midnight SDK call later only requires changing
    midnight_mock.py - nothing else in the codebase should need to know the
    difference.
    """
