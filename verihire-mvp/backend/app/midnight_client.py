...
What this file does
--------------------
This is what routes/candidates.py actually calls to get a "Midnight
proof" - it is the single decision point for "do we call the real
Midnight service, or fall back to the offline mock":
  ...
  - If it IS set, we POST the candidate's private extraction data plus the
    ...
    the same "never let a flaky external dependency take down the demo"
    pattern used in ai_service/app/extraction.py.
...
IMPORTANT honesty note for the team: midnight_service/ contains real
Compact source and a real Node/TypeScript service scaffold written against
Midnight's current public docs and confirmed npm package versions - but it
has NOT been compiled, deployed, or run end-to-end in this environment
(that requires the Compact compiler, a local proof-server Docker
container, Midnight node+indexer RPC access, and a funded testnet wallet -
see midnight_service/README.md). Until your team stands that up and points
MIDNIGHT_SERVICE_URL at it, every proof in this app comes from the offline
mock above, and that's expected, not a bug.
"""
...
def _map_to_circuit_inputs(extraction: ExtractionResult, requirements: JobRequirements) -> dict:
    ...
    those four criteria - e.g. the flagship "job-001" seed job. A job that
    asks for Kubernetes or a Master's degree instead still produces a
    result (missing criteria just default to "not required"/"not held"),
    but that result would NOT reflect the job's actual full requirement
    set. That's a known MVP limitation, not a bug - see the compact file's
    own comment on why a circuit's shape can't be generic the way the
    Python rules engine's is.
    """
